...
## Important: MCP Server Preferred

The vault search and embedding functions are now exposed via the **Vault Tools MCP server** (`.claude/mcp-servers/vault-tools.mjs`). Agents and sessions should use these tools directly instead of calling the CLI commands below. The MCP server includes built-in concurrency control that prevents session crashes from excessive memory usage.

See `.claude/mcp-servers/README.md` for documentation on VaultSearch, VaultStats, and VaultEmbed tools.
